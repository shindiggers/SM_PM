...
import com.catamount.pocketmoney.views.accounts.AccountTypeIconGridActivity;
...
public class Prefs {
    ...
    public static void processDataBase(Context context) {
        ...
        alt_bld.setMessage("/sdcard/PocketMoneyDB/PocketMoneyDB.sql\nYou need to restart after importing").setCancelable(false).setPositiveButton("Import", new AnonymousClass1(cont)).setNegativeButton("Export", new AnonymousClass2(cont));
        alt_bld.create().show();
    }

    public static void importDB(Context c) {
        File dbBackupFile = new File(new StringBuilder(String.valueOf(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath())).append("/PocketMoneyBackup/PocketMoneyDB.sql").toString());
        if (!dbBackupFile.exists()) {
            Log.w(PocketMoney.TAG, "Database backup file does not exist, cannot import.");
        } else if (!dbBackupFile.canRead()) {
            Log.w(PocketMoney.TAG, "Database backup file exists, but is not readable, cannot import.");
        }
        File dbFile = new File(Environment.getDataDirectory() + "/data/" + c.getPackageName() + "/databases/PocketMoneyDB.sql");
        if (dbFile.exists()) {
            dbFile.delete();
        }
        try {
    ...
    public static void exportDB(Context c) {
        File dbFile = new File(Environment.getDataDirectory() + "/data/" + c.getPackageName() + "/databases/PocketMoneyDB.sql");
        if (PocketMoney.IsExternalStorageWritable()) {
            ...
            try {
                ...
                Log.e(PocketMoney.TAG, e.getMessage(), e);
    ...
    private static SharedPreferences getSharedPrefs() {
        if (sharedPrefs == null) {
            sharedPrefs = PreferenceManager.getDefaultSharedPreferences(PocketMoney.getAppContext());
        }
        return sharedPrefs;
    }
    ...
    public static void initialize() {
        if (!getBooleanPref(APP_FIRST_RUN)) {
            Database.populateDatabaseDefaults(PocketMoney.getAppContext());
